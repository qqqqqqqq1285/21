...
class CardWidget(tk.Canvas):
    def __init__(self, parent, card=None, hidden=False):
        super().__init__(parent, width=84, height=124, bg=parent['bg'], highlightthickness=0)
        self.card = card
        self.hidden = hidden
        self.w_current = 80
        self.render()
...
class Game21App(tk.Tk):
    ...
    def show_menu(self):
        ...
        cw = CardWidget(card, {'rank': 'A', 'suit': '♠', 'color': '#2C3E50'}, hidden=True)
        cw.pack(pady=10)
        cw.flip()

        ModernButton(card, "ВЫХОД", self.destroy, width=140, bg=C_BTN_RED, hover_bg=C_BTN_RED_H).pack(pady=10)
