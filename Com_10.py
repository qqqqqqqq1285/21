...
class Game21App(tk.Tk):
    ...
    def show_rules(self):
        ...
        content = (
            ...
            "2. Карты от 2 до 10 дают очки по номиналу. Валет (J), Дама (Q), Король (K) — по 10 очков.\n\n"
            "3. Туз (A) дает 11 очков (или 1 очко при переборе).\n\n"
            "4. Дилер обязан брать карты до 17 очков.\n\n"
            "5. Комбинация «21» с двух карт выплачивается 3 к 2."
        )
    ...
    def next_turn(self):
        ...
        if score == 21:
            self.status_lbl.config(text="КОМБИНАЦИЯ «21»!")
    ...
    def end_round(self):
        ...
        current_bet_val = int(self.current_bet.get().strip())

        if ps > 21:
            res = "ПЕРЕБОР. ВЫ ПРОИГРАЛИ."
        elif p_bj and not d_bj:
            res = "КОМБИНАЦИЯ «21»! (3:2)"
            self.balance += int(current_bet_val * 2.5)
        elif ds > 21 or ps > ds:
            res = "ВЫ ВЫИГРАЛИ!"
            self.balance += current_bet_val * 2
        elif ps == ds:
            res = "НИЧЬЯ"
            self.balance += current_bet_val
        else:
            res = "ДИЛЕР ВЫИГРАЛ."

        self.status_lbl.config(text=res)

        self.btn_next = ModernButton(self.table, "НОВАЯ РАЗДАЧА", self.start_game, width=200, bg="#2980B9", hover_bg="#3498DB")
